...
machine the fast run is **712 cases green, 0 red** — **710** on a clean install, where the
...
a clean install it reads **1179 cases green, 0 red**, with **five** banks reported as
...
> **1230**: 1179 plus 49 host cases plus the 2 of the deploy-history bank. `run-all.sh`
...
712, not 710, because that machine has deployed. `run-all.sh`
